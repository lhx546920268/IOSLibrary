...
- (instancetype)initWithotherButtonTittle:(NSString*) otherButtonTittle
{
    ...
    NSInteger row = 4;
    CGFloat margin = 0.5;
    self = [super initWithFrame:CGRectMake(0, 0, UIScreen.screenWidth, buttonHeight * row + margin * (row - 1))];
    if(self)
    {
...
- (void)buttonDidClick:(UIButton*) btn
{
    ...
    {
        NSRange range = self.textField.sea_selectedRange;
        ...
        self.textField.sea_selectedRange = NSMakeRange(range.location + title.length, 0);
    }
}
...
- (void)deleteText
{
    ...
    if(range.location != 0)
    {
        ...
        self.textField.sea_selectedRange = NSMakeRange(range.location, 0);
    }
    else
    {
        [self stopTimer];
