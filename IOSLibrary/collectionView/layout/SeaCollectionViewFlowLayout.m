...
- (NSArray*)layoutAttributesForElementsInRect:(CGRect)rect
{
    ...
    if(self.itemAlignment == SeaCollectionViewItemAlignmentDefault && !self.sectionHeaderSuspending)
        return attributes;
    ...
    BOOL shouldSuspendDelegate = [self.s_delegate respondsToSelector:@selector(collectionViewFlowLayout:shouldSuspendHeaderForSection:)];

    if(self.sectionHeaderSuspending)
    {
        ...
        {
            BOOL should = YES;
            if(shouldSuspendDelegate)
            {
                should = [self.s_delegate collectionViewFlowLayout:self shouldSuspendHeaderForSection:number.integerValue];
            }
    ...
    for(UICollectionViewLayoutAttributes *attr in attributes)
    {
        ...
        else if (self.sectionHeaderSuspending && [attr.representedElementKind isEqualToString:UICollectionElementKindSectionHeader])
        {
            BOOL should = YES;
            if(shouldSuspendDelegate)
            {
                should = [self.s_delegate collectionViewFlowLayout:self shouldSuspendHeaderForSection:attr.indexPath.section];
            }
